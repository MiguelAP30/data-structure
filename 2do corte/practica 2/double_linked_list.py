...
class DoublyLinkedList:
    ...
    def add_node_in_position(self, position, data):
        ...
        if position == 1:
            ...
            self.length += 1
        print("cantidad de nodos añadiendo en la posicion:", self.length)
    ...
    def remove_node_by_position (self, position):
        ...
        if position == 1:
            print("-------------------")
            print("cantidad de nodos:"+ str(self.length))
    ...
    def remove_node_by_value(self,data):
        if self.head is None:
            ...
            while current.data is not data:
                cont +=1
            self.remove_node_by_position(cont)
            '''
        ...
        if found_duplicates:
            ...
            for value, nodes in values.items():
                if len(nodes) > 1:
                    ...
                    #message += f", ".join([str(node) for node in nodes]) + "\n"
            print(message)            
            return True
        else:
            return False
