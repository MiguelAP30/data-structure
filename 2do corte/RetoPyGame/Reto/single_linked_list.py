class SingleLinkedList:
    ...
    def create_node_sll_ends(self, value):
        if(self.length == 7):
            print("no puede añadir mas de 7 nodos")
        else:
    ...
    #eliminar los elementos duplicados de una lista simplemente enlazada
    def Eliminar_duplicados(self):
        ...
        while (current_node != None):
            ...
            current_node = current_node.next
        lista_unica = []
        for i in array_with_nodes_value:
            if i not in lista_unica:
                lista_unica.append(i)
        self.remove_all_nodes()

        for i in range(len(lista_unica)):
            self.create_node_sll_unshift(lista_unica[i])

    #mostrar de forma continua elementos duplicados

    def Mostrar_continuo_dup(self):
        array_with_nodes_value = list()
        current_node = self.head
        while (current_node != None):
            array_with_nodes_value.append(current_node.value)
        ...
        self.remove_all_nodes()
        for i in range(len(array_with_nodes_value)):
            self.create_node_sll_unshift(array_with_nodes_value[i])
